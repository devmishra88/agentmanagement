...
if($_GET['bulkprinting'] == '1')
{
	...
	if($_GET['customerid'] > 0 || $_GET['customerphone'] != "")
	{
		...
		$CustSQL	= "SELECT cust.* FROM ".$Prefix."customers cust WHERE (id=:id || phone LIKE :phone) AND clientid=:clientid AND deletedon < :deletedon";
		$CustESQL   = array("id"=>(int)$_GET['customerid'],"phone"=>"%".$_GET['customerphone']."%","clientid"=>(int)$_GET['clientid'],"deletedon"=>1);
		...
		if($CustNum > 0)
		{
			...
			$LedgerArr =	GetLedgerByCustomerID($_GET['clientid'],$_GET['customerid'],$StartDate,$EndDate);
...
if(trim(@$_GET['Mode']) != "SendToCustomer")
{
	...
	<body <?if($_GET['bulkprinting'] =='1' && $_GET['frompage'] !='negative' && $_GET['downloadpdf'] !='1'){?>onload='window.print()'<?}?>>
	<?
}
else
{
...
if(!empty($RecordsArr))
{
	...
	if(trim($_GET['startdate_strtotime']) != "" && trim($_GET['enddate_strtotime']) != "")
	{
		...
		{
			$SelectedFilterStr	.= date("j-M-Y", $_GET['startdate_strtotime'])." - ".date("j-M-Y", $_GET['enddate_strtotime']);
		}
		else
		{
			$SelectedFilterStr	.= " / ".date("j-M-Y", $_GET['startdate_strtotime'])." - ".date("j-M-Y", $_GET['enddate_strtotime']);
			...
			<table border='0' cellpadding='0' cellspacing='5' width='100%' align='center' valign='top'>
				<tr>
					<td valign='top' align='left' style="width:50%">
						<div style="font-family:Arial, Helvetica, sans-serif; color:#000;">
							<div style="font-size:<?php echo 18*$fonttobeincrease;?>px;font-weight:bold"><?php echo $AgentName?></div>
						</div>
						<br>
					</td>
					<td valign='top' align='right'>
						<div style="font-family:Arial, Helvetica, sans-serif; color:#000;">
							<div style="font-size:<?php echo 18*$fonttobeincrease;?>px;font-weight:bold">Payment Register</div>
						</div>
						<br>
					</td>
				</tr>
				<tr>
					<td valign='top' align='center' colspan="2">
						<div style="font-family:Arial, Helvetica, sans-serif; color:#000;">
							<div style="font-size:<?php echo 15*$fonttobeincrease;?>px;font-weight:bold"><?php echo $SelectedFilterStr;?></div>
						</div>
					</td>
				</tr>
			</table>
